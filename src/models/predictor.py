...
from models.correct import correction
...
class Translator(object):
	...
	def translate(self,
		...
		with torch.no_grad():
			for batch in data_iter:
				...
				for trans_n, trans in enumerate(translations):
					pred, gold, src = trans
	...
	def _fast_translate_batch(self,
		...
		if self.args.use_dep:
			src_features = torch.cat((src_features, batch.dep.float()), dim=2)
		if self.args.use_frame:
			frame_embeddings = self.model.enc_frame(batch.frame.float().unsqueeze(-1))
			frame_embeddings = self.model.frame_attn(frame_embeddings, frame_embeddings, frame_embeddings, type="self")
			src_features = torch.cat((src_features, frame_embeddings), dim=2)
			# frame_enmbeddings = self.model.enc_frame(batch.frame.float())
			# frame_enmbeddings = frame_enmbeddings.sum(dim=2)
			# src_features = torch.cat((src_features, batch.frame.unsqueeze(-1).float()), dim=2)
		src_features = self.model.enc_out(src_features)
		src_features = self.model.layer_norm(src_features)
